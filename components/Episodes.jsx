...
export default function Episodes({media, selectedSeason = 1, onSeasonChange}) {
  ...
  useEffect(() => {
    ...
    async function getSeason() {
      if (!isAborted) {
        try {
          setIsLoading(true);
          const url = `https://api.themoviedb.org/3/tv/${media.id}/season/${selectedSeason}?language=en-US`;
  ...
  const selectValues = seasonArray.map((seasonNumber, index) => {
    return (
      <option key={`option-${index}`} value={seasonNumber}>
        Season {seasonNumber}
      </option>
    );
  });
  ...
  const episodesElements = seasonData?.episodes?.map((episode, index) => {
    return (
      <div
        ...
        key={`episode-${index}-${episode.season_number}`}
      >
  ...
  return (
    <>
      ...
      <>
        {isLoading && <Loading />}
        <LazyLoadComponent>
          <div className="episodes-wrapper">{episodesElements}</div>
        </LazyLoadComponent>
      </>
    </>
  );
